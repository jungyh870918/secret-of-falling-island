...
> ## ⚠️ 이 문서는 **가로 16:9** 시절에 쓰였다 — 다시 익스포트하기 전에 읽을 것
>
> 2026-08-17 에 내부 해상도가 **320×180 가로 → 942×1674 세로**로 바뀌었다 (`95a1f65`).
> 지금 배포되어 있는 빌드는 **그 이전 것**이라 사이트에서는 아직 가로로 돌아간다.
>
> 세로 빌드를 올리려면 **게임만 다시 익스포트해서는 안 된다.** 사이트 쪽 계약이 같이 깨진다.
>
> | 고칠 곳 | 지금 | 바꿀 값 |
> |---|---|---|
> | `jung-arcade` `data/games.ts` 의 `aspectRatio` | 16:9 | **9:16** (0.5629) |
> | 소개 페이지의 「PC 전용」 안내 | 마우스·키보드 필요 | 세로 + 길게 누르기 지원이라 **모바일이 오히려 맞는 화면비**다. 안내문을 다시 판단할 것 |
> | 스크린샷 4배 확대 | 320×180 원본 전제 | 942×1674 원본은 확대가 아니라 **축소**해야 한다 |
>
> 아래 본문에서 320×180 · 16:9 로 적힌 수치는 전부 이 시점 기준의 기록이다.
> 스레드 끄기 · COOP/COEP · 세이브(IndexedDB) · 예약 키 같은 나머지 결론은 **해상도와 무관하게 유효하다.**
...
| ~~내부 해상도 320×180~~ | **이미 바뀌었다** (2026-08-17 → 942×1674 세로). 사이트가 아직 16:9 컨테이너로 감싸고 있으므로, 다시 익스포트할 때 `jung-arcade` 의 `data/games.ts` 에서 `aspectRatio` 를 **9:16 으로 반드시 같이 고친다.** 문서 첫머리 경고 참고 |
