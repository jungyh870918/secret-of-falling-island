...
익스포트 → 로컬 서버 → 헤드리스 Chrome → 7건 검사까지 한 번에 한다.
...
지금은 **모바일에서 전혀 플레이할 수 없다.** 좌클릭·우클릭 구분과 키보드가
전제이기 때문이다 (`README.md` 조작 표).

사이트는 모바일에서 「PC 전용」 안내를 띄우고 있어 사용자가 오해하지는 않는다.
다만 웹게임의 상당수 유입이 모바일이라, 지원하면 도달 범위가 크게 는다.

넣는다면 필요한 것:
- 우클릭(기본 동작) 대체 — 길게 누르기 또는 대상 선택 후 확인 버튼
- 동사 패널 터치 목표 크기 확보 (지금은 320×180 기준이라 폰에서 매우 작다)
- 커서 개념 제거 또는 대체

같은 사이트의 다른 게임(`three-kingdoms`)이 이 문제를 이렇게 풀었다 —
**터치 기기를 감지하면 스테이지 안의 조작 목표물을 키우고, 크기를 보장해야
하는 버튼은 스테이지 밖에 실제 픽셀로 둔다.** 참고할 만하다.

#### P2-2. 전체 화면을 게임 안에서

F11 을 웹에서 빼면 게임 안에는 전체 화면 수단이 없어진다.
사이트가 페이지 쪽 버튼을 제공하지만, 게임 안 설정 메뉴에도 하나 있으면 좋다.
Godot 웹에서 `DisplayServer.window_set_mode(WINDOW_MODE_FULLSCREEN)` 는
사용자 조작에서 비롯된 호출이어야 동작한다.

#### P2-3. 빌드 크기

지금 wasm 이 37.6MB(gzip 10.2MB)다. 사이트 저장소에 그대로 커밋되어 있어,
**다시 익스포트할 때마다 40MB 가 저장소 이력에 쌓인다.**

`exclude_filter` 로 `tests/`, `docs/`, `tools/` 는 이미 뺐다.
더 줄이려면 폰트가 후보다 — `assets/fonts/` 의 Galmuri 3종이 5MB 를 차지한다
(`README.md` 폰트 절). 다만 한글 11,172자를 유지하는 판단에는 이유가 있으므로
함부로 줄이지 말 것.

자주 다시 빌드하게 되면 별도 호스팅으로 옮기는 편이 낫다. 사이트 쪽은
`data/games.ts` 의 `playUrl` **한 줄만** 바꾸면 되도록 되어 있다.

#### P2-4. 웹 빌드 자동 점검 — ✅ 처리됨

`tests/web_check.sh` 가 익스포트 성공 여부를 넘어 실제 브라우저 동작까지 본다.
자세한 내용은 위 P1 절 「재현 방법」.
